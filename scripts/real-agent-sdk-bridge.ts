...
interface RealAgentSdkBridgeRequest {
...
interface RealAgentSdkBridgeResponse {
  success: boolean;
  content: string;
...
class RealClaudeAgentSdkBridge {
  ...
  async handleRequest(
    request: RealAgentSdkBridgeRequest
  ): Promise<RealAgentSdkBridgeResponse> {
    try {
  ...
  private buildFullPrompt(request: RealAgentSdkBridgeRequest): string {
  ...
  private buildSdkOptions(request: RealAgentSdkBridgeRequest): Options {
    ...
    // 如果需要禁止某些工具
    if (
      !this.permissions.allow_all &&
      this.permissions.deny_patterns.length > 0
    ) {
      const disallowedTools = this.extractDisallowedTools(
        this.permissions.deny_patterns
      );
  ...
  private mapToolsToSdkFormat(tools: string[]): string[] {
    // SDK 的工具名称映射
  ...
  private extractDisallowedTools(denyPatterns: string[]): string[] {
...
保持代码风格一致性。`,
  ...
  private async processQueryStream(
    queryGenerator: AsyncGenerator<SDKMessage, void>
  ): Promise<RealAgentSdkBridgeResponse> {
    let finalContent = "";
    let turnCount = 0;
    let totalTokens = 0;
  ...
  private trackToolUse(toolName: string, input: any): void {
...
async function main() {
  ...
  rl.on("close", async () => {
    try {
      const request: RealAgentSdkBridgeRequest = JSON.parse(inputData);
      const response = await bridge.handleRequest(request);
      console.log(JSON.stringify(response));
      process.exit(0);
    } catch (error: any) {
      const errorResponse: RealAgentSdkBridgeResponse = {
        success: false,
        content: "",
        code_changes: [],
        tool_uses: [],
