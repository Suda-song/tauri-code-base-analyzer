...
interface BridgeResponse {
  success: boolean;
  content: string;
  session_id: string; // 返回当前的 session_id
  turn_count: number;
...
class PersistentMcpAgentBridge {
  ...
  async executeQuery(request: BridgeRequest): Promise<BridgeResponse> {
    console.error("\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    ...
    try {
      ...
      if (sessionToResume) {
        console.error(
          `🔄 使用 resume 功能恢复会话: ${sessionToResume.substring(0, 8)}...\n`
        );
      }
  ...
  resetSession() {
    ...
    if (this.currentSessionId) {
      console.error(
        `   旧 Session ID: ${this.currentSessionId.substring(0, 8)}...`
      );
    }
