    ...
    async execute(request) {
        ...
        try {
            ...
            for await (const message of response) {
                ...
                // 打印原始消息用于调试
                console.error(`   原始消息: ${JSON.stringify(message).substring(0, 200)}...`);
                if (message.type === "assistant") {
                    const assistantMsg = message.message;
                    for (const block of assistantMsg.content) {
                        if (block.type === "text") {
                            finalContent += block.text;
                            console.error(`  📝 文本内容: ${block.text.substring(0, 100)}...`);
                        }
                    ...
                    // 其他消息类型（可能包含错误）
                    console.error(`  ℹ️  其他消息: ${JSON.stringify(message).substring(0, 200)}...`);
                }
            }
            const result = {
                success: true,
